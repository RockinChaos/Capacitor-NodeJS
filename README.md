...
| **`nodeArgs`**  | <code>string[]</code>           | A list of Node.js startup arguments passed to the runtime on launch. When using manual start mode, arguments passed to `NodeJS.start()` take precedence over this config if defined.                                                      | <code>[]</code>       | 1.0.0-beta.12 |
...
| **`args`**    | <code>string[]</code>                       | A list of string arguments passed to the Node.js runtime. If not specified, falls back to the `nodeArgs` field of the global plugin configuration.                                                                                                                       | 1.0.0 |
